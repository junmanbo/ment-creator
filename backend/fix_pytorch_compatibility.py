...
def main():
    print("🔧 PyTorch 2.6+ 호환성 문제 해결 스크립트")
    print("=" * 50)
    ...
    if torch_version:
        try:
            ...
            if major > 2 or (major == 2 and minor >= 6):
                ...
                print("⚠️  PyTorch 2.6+ 감지 - 호환성 문제 가능성 있음")
            else:
    ...
    if needs_downgrade:
        print("\n🔧 해결 방법:")
        print("1. PyTorch 다운그레이드 (권장)")
        print("   uv add 'torch>=2.0.0,<2.6.0' 'torchaudio>=2.0.0,<2.6.0'")
        print("   또는")
        print("   pip install 'torch>=2.0.0,<2.6.0' 'torchaudio>=2.0.0,<2.6.0'")
        print()
        
        # 자동 다운그레이드 제안
        print("🤖 자동으로 다운그레이드하시겠습니까? (y/n): ", end="")
        ...
        if response in ['y', 'yes', 'ㅇ']:
            ...
            if uv_available:
                print("🔄 uv를 사용하여 다운그레이드...")
                success, stdout, stderr = run_command(
                    "uv add 'torch>=2.0.0,<2.6.0' 'torchaudio>=2.0.0,<2.6.0'"
                )
            else:
                print("🔄 pip를 사용하여 다운그레이드...")
                success, stdout, stderr = run_command(
                    "pip install 'torch>=2.0.0,<2.6.0' 'torchaudio>=2.0.0,<2.6.0'"
                )
